  ...
  export class TableComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private initializePlayerRole() {
      const storedRolPlayer = localStorage.getItem('rolPlayer');
      this.isPlayerRole = storedRolPlayer !== 'espectador';
    }
    ...
    private subscribeToCardSelection() {
      this.cardsContainer.cardSelected.subscribe((selectedCard: any) => {
        const selectedIndex = this.selectedCards.indexOf(this.selectedCard);
        if (selectedIndex !== -1) {
        ...
        this.socket$.emit('cardSelected', { userId: this.userId, selectedCard });
        this.socket$.on('cardsUpdated', (selectedCards) => {
    ...
    handleRolPlayer(rolPlayer: string) {
      this.isPlayerRole = rolPlayer.toLowerCase() !== 'espectador';
    ...
    checkSelectedCardForUser(userId: string): void {
      const selectedCard = localStorage.getItem('selectedCard');
      if (selectedCard && selectedCard === userId) {
        const cardElement = document.querySelector('.user-card-selected');
        if (cardElement) {
          cardElement.classList.add('user-card-selected');
        }
      }
    }
